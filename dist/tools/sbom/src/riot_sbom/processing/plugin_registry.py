    ...
    for filename in os.listdir(directory):
        if filename.endswith('.py') and filename != '__init__.py':
            if add_to_search_path and directory.absolute().as_posix() not in sys.path:
                ...
                logging.debug(f"Adding {directory.absolute().as_posix()} to sys.path")
...
def register_plugin(name: str, plugin: Plugin) -> None:
    ...
    :type plugin: Plugin
    """
    logging.debug(f"Registering plugin: {name} to registry: {_plugin_registry}")
    if not name:
    ...
    logging.info(f"Registered plugin: {name} - {plugin.get_description()}")


def _load_plugins_from_module(module: ModuleType) -> None:
    logging.debug(f"Loading plugins from module: {module.__name__}")
    for name, obj in vars(module).items():
        logging.debug(f"Checking {name} in module {module.__name__}")
        logging.debug(f"Object type: {type(obj)}")
        if isinstance(obj, type):
            logging.debug(f"Object {name} is a class")
            logging.debug(f"Class {name} is a subclass of Plugin: {issubclass(obj, Plugin)}")
            logging.debug(f"Class {name} is not Plugin: {obj is not Plugin}")
        if isinstance(obj, type) and issubclass(obj, Plugin) and obj is not Plugin:
            try:
                plugin_instance = obj()
            except Exception as e:
                import traceback
                logging.warning(f"Failed to instantiate plugin {name}: {traceback.format_exc()}")
                continue
