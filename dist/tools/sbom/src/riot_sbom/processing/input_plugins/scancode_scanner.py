...
def _scancode_scan_file(pth: str):
...
class ScancodeScanner(Plugin):
    ...
    def run(self, app_info: AppInfo, _) -> AppInfo:
        ...
        scancode_args = (
            False, # from_json: bool
            False, # strip_root: bool
            True, # full_root: bool
            10000, # max_in_memory: int
            -1, # processes: int
            120, # timeout: int
            True, # quiet: bool
            False, # verbose: bool
            1, # max_depth: int
            None, # echo_func: Unknown | None
            False, # timing: bool
            False, # keep_temp_files: bool
            True, # return_results: bool
            False, # return_codebase: bool
            False, # test_mode: bool
            False, # test_slow_mode: bool
            False, # test_error_mode: bool
            None, # pretty_params: Unknown | None
        )
                    ...
                    for c in scan_result['files'][0]['copyrights']:
                        ...
                        if m:
                            ...
                            path_scan_result.copyrights.append(CopyrightInfo(
                                holder=holder,
                                years=years,
                                declaration_type=CopyrightDeclarationType.TEXT_MATCHED,
                                url=None))
                if (scan_result.get('files') and
        ...
        for file in app_info.files:
            if (file.path in scancode_output_transformed
                ...
                logging.info(f"Scancode did not return any data for file: {file.path}")
        return app_info
