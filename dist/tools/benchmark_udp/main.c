...
static void _print_stats(list_node_t *head, struct timeval *now)
{
    ...
    for (list_node_t* n = head->next; n; n = n->next) {
        ...
        unsigned bw = (node->count_rx * (payload_len + sizeof(benchmark_msg_ping_t)) * MS_PER_SEC)
                    / (1 + _tv_diff_msec(now, &node->first_seen));
        ...
        printf("%*s\t%4u b/s\t%u\t%u (%u%%)",
                                ...
                                success_rate);
        if (!one_way) {
            unsigned success_rate_rt = node->count_rt
                                     ? (100 * node->count_rt) / node->count_rt
                                     : 0;
...
static void dispatch_loop(int sock)
{
    ...
    puts("entering loop…");
    while (1) {
        uint8_t buffer[BENCH_PAYLOAD_SIZE_MAX];
