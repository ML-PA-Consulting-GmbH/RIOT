...
if (IS_ACTIVE(CONFIG_USE_LOOPBACK_MODE)) {
    ...
    if (IS_ACTIVE(MCP2515_RECV_FILTER_EN)) {
        ...
        filter[1].can_id = 0x002;   /* messages with CAN ID 0x002 will be received in mailbox 1 */
        filter[1].target_mailbox = 1;
        filter[2].can_mask = 0x7FF;
        filter[2].can_id = 0x003;    /* messages with CAN ID 0x003 will be received in mailbox 0 */
        filter[2].target_mailbox = 0;
        filter[3].can_mask = 0x7FF;
        filter[3].can_id = 0x004;   /* this filter won't be applied. Reason is no space found in the first mailbox as it supports only two filters */
        filter[3].target_mailbox = 0;
