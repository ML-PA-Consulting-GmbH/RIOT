...
static int shell_is2dh12_set_powermode(int argc, char **argv)
{
    unsigned powermode = UINT_MAX;

    if (argc > 1) {
        powermode = atoi(argv[1]);
    }

    if (powermode > LIS2DH12_POWER_HIGH) {
        printf("usage: %s <mode>\n", argv[0]);
        puts("where <mode> is:");
        puts("\t0: off");
        puts("\t1: low");
    ...
    lis2dh12_set_powermode(&dev, powermode);
...
static const shell_command_t shell_commands[] = {
    ...
    { "powermode", "Get/Set power mode", shell_is2dh12_set_powermode },
    { "rate", "Get/Set power mode", shell_is2dh12_set_rate },
    { "scale", "Get/Set power mode", shell_is2dh12_set_scale },
    { NULL, NULL, NULL },
};
