...
static void _error_code_handling(candev_t *candev, uint8_t error_code)
{
    ...
    switch (error_code) {
        ...
        case CANDEV_SAMD5X_NO_CHANGE_ERROR:
            /* no can event happend -> unchanged */
            break;
...
/* PED and PEA must be handled at the same time as reading PSR destroyes the LEC and DLEC */
static void _pe_isr(candev_t *candev, uint32_t irq_reg ){
    ...
    /* LEC is not NO_CHANGE when retrived -> save error_code */
    if (lec != CANDEV_SAMD5X_NO_CHANGE_ERROR) {
       dev->state.last_error_code = lec;
    }

    /* DLEC is not NO_CHANGE when retrived -> save error_code */
