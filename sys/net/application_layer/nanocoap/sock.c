                         ...
                         void *response, size_t max_len)
{
    ...
    ssize_t hdr_len = coap_build_udp_hdr(buffer, sizeof(buffer), type, NULL, 0, COAP_METHOD_GET,
                       ...
                       void *response, size_t max_len)
{
    ...
    uint16_t lastonum = 0;
    ssize_t hdr_len = coap_build_udp_hdr(buffer, sizeof(buffer), type, NULL, 0, code, nanocoap_sock_next_msg_id(sock));
    assume(hdr_len > 0);
    pktpos += hdr_len;
...
ssize_t nanocoap_sock_delete(nanocoap_sock_t *sock, const char *path)
{
    ...
    ssize_t hdr_len = coap_build_udp_hdr(buffer, sizeof(buffer), COAP_TYPE_CON, NULL, 0,
                        ...
                        _block_ctx_t *ctx)
{
    ...
    case COAP_SOCKET_TYPE_UDP:
    case COAP_SOCKET_TYPE_DTLS:
        hdr_len = coap_build_udp(&pkt, buf, len, COAP_TYPE_CON, token, token_len,
                                 COAP_METHOD_GET, nanocoap_sock_next_msg_id(sock));
        break;
#if MODULE_NANOCOAP_TCP
    case COAP_SOCKET_TYPE_TCP:
        hdr_len = coap_build_tcp(&pkt, buf, len, token, token_len, COAP_METHOD_GET);
        break;
#endif
#if MODULE_NANOCOAP_WS
    case COAP_SOCKET_TYPE_WS:
        hdr_len = coap_build_ws(&pkt, buf, len, token, token_len, COAP_METHOD_GET);
        break;
#endif
    default:
        assert(0);
                                ...
                                coap_request_cb_t callback, void *arg)
{
    ...
    ssize_t hdr_size =  coap_build_udp_hdr(buf, sizeof(buf), COAP_TYPE_CON, NULL, 0, req->method,
