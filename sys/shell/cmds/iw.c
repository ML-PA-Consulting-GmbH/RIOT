...
static void _wifi_scan_cb(void *netif, const wifi_scan_list_t *result)
{
    (void)netif;
    _aps.numof = 0;
    const wifi_scan_list_t *list = result;
    const wifi_scan_list_node_t *node = (wifi_scan_list_node_t *)list->head.next;
    while (_aps.numof < ARRAY_SIZE(_aps.ap) && node) {
        _aps.ap[_aps.numof++] = node->result;
        node = (wifi_scan_list_node_t *)node->node.next;
    }
    _list_ap();
    mutex_unlock(&_sync);
}
